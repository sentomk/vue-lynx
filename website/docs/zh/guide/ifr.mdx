...
下表把三种产品配置明确分开。FCP 来自 Lynx for Web 的真实 Web Worker，hello-world 无 CPU 节流测量（十个 example 的收益中位为 −24%）；渲染成本来自约 1000 元素动态内容场景的 V8 `--jitless` warm 中位；TTI 和 gzip 是全示例扫描相对 No IFR 归一化后的中位数。这些列来自不同轮次，不应视为同一条端到端 trace。
...
1. **IFR 的收益是真实的、结构性的，且与参照实现同级。**在有真实线程边界的
   Lynx for Web 上，十个 example 的 FCP 全部改善——−9% 到 −29%，中位
   **−24%**，与 ReactLynx 在同一 harness 上的 −26…−31% 属同一量级。收益
   来源是把后台线程启动 + IPC 从关键路径上移除——这也是单进程 benchmark
   看不到它的原因。
2. **元素模板攻击的正是 IFR 新增的那笔成本。**主线程同步渲染便宜 7–15×，
   ops 载荷缩小 3–1000×（一个 1400 元素的静态屏从 78 KB 缩到 69 字节）。
   满速小屏上这不动 FCP——渲染本来就只有几毫秒——但在渲染成本要紧的地方
   ET 全面领先：慢 CPU、大屏（302 节点的 gallery 在 4× 节流下 −7% vs
   −1%）、原生解释器；它还让 hydration 本身变便宜。这种不对称性——最差
   不过噪声、要紧处必赢——就是 ET 默认随 IFR 开启的原因。
3. **代价是 bundle 体积，它也定义了反面画像。**`main.lynx.bundle` gzip
   中位增长 ×2.26，可交互前的串行工作量最多 ×1.36（真机上与后台启动
   重叠）。大 bundle 配纯骨架、fetch 驱动的首屏在 web 上可能反转
   （hackernews 节流下 +19%）——因为 web 把 bundle 解析付在 FCP 路径上；
   原生预编译字节码会把这一项压小。
...
| 你的首屏 | 建议 |
| --- | --- |
| 内容优先、由同步数据渲染 | **开 `enableIFR: true`**（自带 ET）——这是目标画像，中位 −24% FCP |
| 数据驱动但初始数据同步可得 | 开 |
